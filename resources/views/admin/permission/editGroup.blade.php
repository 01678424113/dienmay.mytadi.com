...
@section('pagecontent')
    ...
    <div class="portlet light">
        ...
        <div class="portlet-body form">
            {!! Form::open(['action' => ['Admin\PermissionController@doEditGroup', $group->group_id], 'method' => 'POST', 'id' => 'group-form']) !!}
            <div class="form-body">
                <div class="form-group">
                    <label class="control-label">Tên nhóm<span class="required"> * </span></label>
                    <input type="text" class="form-control" name="txt-name" value="{{ $group->group_name }}">
                </div>
                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <div class="input-group group-permission">
                                        <div class="icheck-list">
                                            <label role="button">
                                                <input type="checkbox" value="{{ $func->function_key }}" name="cb-functions[]"
                                                       {{ isset($group->functions[$func->function_key]) ? 'checked' : '' }}
                                                       class="icheck cb-function" data-checkbox="icheckbox_square-blue">
                                                <span class="font-bold font-13">{!! $func->function_name !!}</span>
                                            </label>
                                        </div>
                                        <div class="form-group m-l-25 m-t-15">
                                            <div class="input-group">
                                                <div class="icheck-list">
                                                    ...
                                                    <label role="button">
                                                        <input type="checkbox" value="view" name="cb-{{ $func->function_key }}-actions[]"
                                                               {{ isset($group->functions[$func->function_key]) && in_array('view', $group->functions[$func->function_key]) ? 'checked' : '' }}
                                                               class="icheck cb-action" data-checkbox="icheckbox_minimal-blue"> Xem
                                                    </label>
                                                    <label role="bu                                                       tton">
                                                        <input type="checkbox" value="add" name="cb-{{ $func->function_key }}-actions[]"
                                                               {{ isset($group->functions[$func->function_key]) && in_array('add', $group->functions[$func->function_key]) ? 'checked' : '' }}
                                                               class="icheck cb-action" data-checkbox="icheckbox_minimal-blue"> Thêm
                                                    </label>
                                                    <label role="button">
                                                        <input type="checkbox" value="edit" name="cb-{{ $func->function_key }}-actions[]"
                                                               {{ isset($group->functions[$func->function_key]) && in_array('edit', $group->functions[$func->function_key]) ? 'checked' : '' }}
                                                               class="icheck cb-action" data-checkbox="icheckbox_minimal-blue"> Sửa
                                                    </label>
                                                    <label role="button">
                                                        <input type="checkbox" value="delete" name="cb-{{ $func->function_key }}-actions[]"
                                                               {{ isset($group->functions[$func->function_key]) && in_array('delete', $group->functions[$func->function_key]) ? 'checked' : '' }}
                                                               class="icheck cb-action" data-checkbox="icheckbox_minimal-blue"> Xóa
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
            ...
            <div class="form-actions" style="clear: both;">
                ...
                <a href="{{URL::action('Admin\PermissionController@listGroup')}}" class="btn red-soft uppercase">Hủy bỏ</a>
            </div>
            {!! Form::close() !!}
        </div>
    </div>
